...
import com.hopeofseed.hopeofseed.DataForHttp.AddNewProblem;
...
public class PublishProblem extends AppCompatActivity implements View.OnClickListener, NetCallBack {
    ...
    private void SubmitProblem() {
        Log.e(TAG, "getData: 获取经销商数据");
        HashMap<String, String> opt_map = new HashMap<>();
        opt_map.put("ProblemTitle", et_title.getText().toString());
        opt_map.put("ProblemContent", et_content.getText().toString());
     ...
     /*   new Thread(new Runnable() {
            @Override
            public void run() {
                AddNewProblem addNewProblem = new AddNewProblem();
                addNewProblem.ProblemTitle = et_title.getText().toString();
                addNewProblem.ProblemContent = et_content.getText().toString();
                addNewProblem.ProblemVariety = et_variety.getText().toString();
                Boolean bRet = addNewProblem.RunData();
